    ...
    context "given two different versions of the same non-custom class" do
      it "returns a message along with the diff" do
        ...
        expected_output = <<~STR.strip
          ...
          #{
            colored do
              alpha_line %(Expected: #<SuperDiff::Test::Player:#{SuperDiff::Helpers.object_address_for(expected)} @character="mirage", @handle="martymcfly", @health=0.3, @inventory=["flatline", "purple body shield"], @shields=0.6, @ultimate=0.8>)
              beta_line  %(  Actual: #<SuperDiff::Test::Player:#{SuperDiff::Helpers.object_address_for(actual)} @character="lifeline", @handle="docbrown", @health=0.3, @inventory=["wingman", "mastiff"], @shields=0.6, @ultimate=0.8>)
            end
          }
    ...
    context "given two completely different kinds of non-custom objects" do
      it "returns a message along with the diff" do
        ...
        expected_output = <<~STR.strip
          ...
          #{
            colored do
              alpha_line %(Expected: #<SuperDiff::Test::Item:#{SuperDiff::Helpers.object_address_for(expected)} @name="camera", @quantity=3>)
              beta_line  %(  Actual: #<SuperDiff::Test::Player:#{SuperDiff::Helpers.object_address_for(actual)} @character="Jon", @handle="mcmire", @health=4, @inventory=["sword"], @shields=11.4, @ultimate=true>)
            end
          }
        STR
